...
import { Jin10 } from '~/features/jin10/types'
...
export async function crawlOneEvent(params: z.infer<typeof schema>) {
  ...
  const dayAt = days(input.dateAt)
  const isValid = dayAt.isValid()

  if (!isValid) {
    throw new Error(
      `[expect] params.dateAt satisfies string [toMatch] YYYY-MM-DD [retrieve] ${input.dateAt} [typeof] ${typeof input.dateAt}`,
    )
  }
  ...
  ourEvents = ourEvents.map((event) => {
    return {
      ...event,
      // the data fetched by the web scraper here is in +0800 timezone,
      // so i need to convert it to utc timezone before saving to the database for consistent time zone handling
      publish_at: days(event.publish_at).add(-8, 'hours').toISOString(),
    }
  })
  ...
  ourEvents = ourEvents.toSorted(
    (a, b) =>
      (days(a.publish_at).unix() ?? 0) - (days(b.publish_at).unix() ?? 0),
  )
